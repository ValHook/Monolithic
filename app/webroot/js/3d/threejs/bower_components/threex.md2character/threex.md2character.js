...
THREEx.MD2Character.prototype.load		= function ( config )
{
	...
	var okbody = false;
	var geometrybody;
	for (var h = 0; h<ramod.length;h++) {
		if (h.name == config.body) {
			okbody = true;
			geometrybody = ramod[h].geometry;
			break;
		}
	}
	if (okbody == false) {
	loader.load( config.baseUrl + config.body, function( geometry ) {
		...
		ramod.push({name: config.body, geometry: geometry});
		this._checkLoadingComplete();
	}.bind(this));
	} else {
		geometrybody.computeBoundingBox();

		var mesh	= createPart( geometrybody, this.texturesBody[ 0 ] );
		mesh.scale.set( this.scale, this.scale, this.scale );
		mesh.position.y	= 0.5
		...
		this.meshBody		= mesh;
		this._curAnimation	= geometrybody.firstAnimation;
	...
	var generateCallback = function( index, name ){
		return function( geometry ) {
			...
			var okweapon = false;
			for (var k =0; k< ramod.length;k++) {
				if (ramod[k].name == name) {
					okweapon = true;
					break;
				}
			}
			if (okweapon==false)
				ramod.push({geometry: geometry, name: name});
	...
	for ( var i = 0; i < config.weapons.length; i ++ ) {
		...
		var okweapon = false;
		for (var k =0; k< ramod.length;k++) {
			if (ramod[k].name == config.weapons[ i ][ 0 ]) {
				okweapon = true;
				callback(ramod[k].geometry);
				break;
			}
		}
		if (okweapon == false)
...
//	console.log(this.nLoadInProgress);
...
THREEx.MD2Character.prototype._loadTextures	= function( baseUrl, textureUrls )
{
	...
	for( var i = 0; i < textureUrls.length; i ++ ){
		...
		var ok = true;
		var texture;
		for (var j = 0; j < ratex.length; j++) {
			if (ratex[j].url == url) {
				texture = ratex[j].image;
				callback();
				ok = false;
				break;
			}
		...
		if (ok == true) {
			texture		= THREE.ImageUtils.loadTexture( url, THREE.UVMapping, callback);
			ratex.push({url: url, image: texture});
		}
